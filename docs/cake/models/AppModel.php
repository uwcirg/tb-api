...
//                  $this->log("baseModelName: " . $baseModelName, LOG_DEBUG);
                    $subclassModelName =
                        $baseModelName .
                        Inflector::camelize(INSTANCE_ID);
                    //eg "SurveySessionP3p"
                ...
                foreach ($val[$name] as $innerKey => $innerVal){
                    if(substr($innerKey,-1*strlen($i18nPostfix))==$i18nPostfix){
                        $default_eng = substr($innerKey,0,-1*strlen($i18nPostfix) - 1);
                            if(
                                !isset($results[$key][$name][$innerKey]) and
                                isset($results[$key][$name][$default_eng]) and
                                $results[$key][$name][$default_eng]
                            )
...
//        $this->log(__CLASS__ . '.' . __FUNCTION__ . "($userId), trace:" . Debugger::trace() . "\n\n", LOG_DEBUG);
        ...
        $queryStr = "SELECT timezone from sites 
            ...
            LIMIT 1";
        $result = $this->query($queryStr); //FIXME
    ...
    function afterFind($results, $primary=false) {
        ...
        if (array_key_exists(0, $results) and is_array($results[0])){
            ...
            foreach ($results as $index => &$rowData) {
                ...
                // Attributes without a corresponding table (eg SQL aliases)
                if (array_key_exists(0, $rowData)){
                    foreach ($rowData[0] as $label => $value){

                        // Allow implicit setting of table and attribute with "__"
                        if (strpos($label, '__') !== false){
                            list($table, $attribute) = explode('__', $label);
                            $rowData[$table][$attribute] = $value;
                            unset($rowData[0][$label]);
                        }
                    }
                    // Unset if empty
                    if (!$rowData[0]){
                ...
                if (
                    array_key_exists('Patient', $rowData) and
                    array_key_exists('PatientExtension', $rowData) and
                    array_key_exists('patient_id', $rowData['PatientExtension'])
                )
                    $rowData['Patient'] = array_replace_recursive(
                        $rowData['Patient'], $rowData['PatientExtension']
                    );
            }
        }
        ...
        // Single result
            ...
            array_key_exists('Patient', $results) and
            array_key_exists('PatientExtension', $results) and
            array_key_exists('patient_id', $results['PatientExtension'])
        ){
            $results['Patient'] = array_replace_recursive(
                $results['Patient'], $results['PatientExtension']
            );
        }
